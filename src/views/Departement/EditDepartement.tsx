...
const EditDepartementDialog: React.FC<Props> = ({ open, handleClose, departement }) => {
    ...
    onSubmit: async (formValues) => {
      try {
        if (editedDepartement) {
          const response = await axios.post(`http://localhost:5000/departement/${editedDepartement._id}/addManager?departementId=${editedDepartement._id}`, formValues);
    ...
    <>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogActions>
          ...
          <Button type="submit" color="primary" onClick={formik.handleSubmit}>Enregistrer les modifications</Button>
        </DialogActions>
      </Dialog>
    </>
  );
...
export default EditDepartementDialog;
